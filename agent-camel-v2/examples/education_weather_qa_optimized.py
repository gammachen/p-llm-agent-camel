...
示例：
{"question": "地球上天气变化的主要能量来源是什么？", "answer": "太阳"}
...
4. 只给出你认为的正确答案"""
    ...
    def play_round(self, round_num: int) -> Dict[str, Any]:
        ...
        # AI助手出题
        print("🤖 AI助手正在出题...")
        question_msg = BaseMessage.make_user_message("系统", f"请出第{round_num}个天气知识题目，必须输出JSON格式")
    ...
    def play_game(self, max_rounds: int = 15, min_rounds: int = 8, threshold: float = 0.35) -> Dict[str, Any]:
        ...
        # 成绩评级 - 更合理的评级标准
        if rate >= 0.75:
            grade, comment = "优秀", "天气知识大师！"
        elif rate >= 0.55:
            grade, comment = "良好", "气象知识不错！"
        elif rate >= 0.35:
            grade, comment = "及格", "继续加油！"
        else:
            grade, comment = "需要努力", "多学习天气知识！"
